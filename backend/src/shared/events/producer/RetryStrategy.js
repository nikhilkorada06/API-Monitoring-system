...
export function isErrorRetryable(err) {
    ...
    if (errCode === 'ENOTFOUND') return true;  // Network-related error such as DNS resolution failure it is mostly temporary so we can retry

    return RETRYABLE_PATTERNS.some(
        (p) => errMsg.includes(p.toLowerCase()) || errCode.includes(p.toUpperCase())
    )
}
...
export class RetryStrategy {
     ...
     */
    delay(attempt) {
        const exponentialBackoff = this.baseDelayMs * Math.pow(2, attempt); // example : baseDelayMs=200, attempt=3 => 200 * 2^3 = 1600ms
        const capped = Math.min(exponentialBackoff, this.maxDelayMs);       // example : capped = min(1600, 1000) = 1000ms

        const jitterRange = capped * this.jitterFactor;           // example : jitterRange = 1000 * 0.3 = 300ms
        const jitter = (Math.random() - 0.5) * 2 * jitterRange;     // example : jitter = (random value between -300 and 300)

        return Math.max(0, Math.round(capped + jitter)); // example : final delay = 1000 + jitter (between 700 and 1300ms)
    }
